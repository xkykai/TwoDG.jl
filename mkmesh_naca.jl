...
# Open the mesh file
filename = "naca0012.msh"

function read_gmsh_mesh(filename)
    # Initialize Gmsh
    gmsh.initialize()
...
p, t = read_gmsh_mesh(filename)
#%%
p_unique, t_unique = fixmesh(p, t)

scatter(p_unique[:, 1], p_unique[:, 2], markersize=3)
...
plocal, tlocal = uniformlocalpnts(2)

mesh = TwoDG.Mesh(p_unique, t_unique, f, t2f, fcurved, tcurved, 3, plocal, tlocal)

new_mesh = createnodes(mesh)
#%%
fig = Figure(size=(2000, 1500))
ax = Axis(fig[1, 1], aspect=DataAspect())
dgnodes = new_mesh.dgnodes
tlocal = new_mesh.tlocal
...
t = mesh.t
for i in 1:size(t, 1)
    poly!(ax, new_mesh.p[t[i, :], 1], new_mesh.p[t[i, :], 2], strokewidth=2, strokecolor=:black, color=(:white, 0))
end

display(fig)
save("naca0012.png", fig, px_per_unit=4)
#%%